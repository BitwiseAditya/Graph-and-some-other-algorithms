...
of each list j denotes there is edge between i and j , second integers corresponds to
the weight of that  edge . You are given the source vertex S and You to Find the shortest
distance of all the vertex's from the source vertex S. You have to return a list of integers
...
Output:
0 9
Explanation: The source vertex is 0. Hence, the shortest
...
vector<int> dijkstra(int V, vector<vector<int>> adj[], int S)
{
    ...
    while (!pq.empty())
    {
        ...
        pq.pop();
        for (auto &nums : adj[vertex])
        {
            if (distance[nums[0]] > weight + nums[1])
            {
                distance[nums[0]] = weight + nums[1];
                pq.push({distance[nums[0]], nums[0]});
            }
        }
    }
    return distance;
